...
export class TmdbService {
  ...
  titleSearch(title: string) {
    ...
    this.http.get<IMovie[]>('https://ng-pmc.herokuapp.com/api/search', options).subscribe((results) => {
  ...
  addMovieToWatchlist() {
    ...
    this.http.post<IWatchlist>(`https://ng-pmc.herokuapp.com/api/search`, payload)
  ...
  archiveMovie(movieId: number, archiveForm: FormGroup) {
    ...
    this.http.post<IWatchlist>(`https://ng-pmc.herokuapp.com/api/watchlist/${watchlistId}/${movieId}/archive`, payload).subscribe((updatedWatchlist) => {
      let updatedWatchlists = this.watchlistService.watchlists.value;
      const watchlistIndex = updatedWatchlists.findIndex(watchlist => watchlist._id === updatedWatchlist._id);
      updatedWatchlists[watchlistIndex] = updatedWatchlist;
      this.watchlistService.watchlists.next(updatedWatchlists);
  ...
  deleteMovieFromWatchlist(movieId: number) {
    ...
    this.http.delete<IWatchlist>(`https://ng-pmc.herokuapp.com/api/watchlist/${watchlistId}/${movieId}`).subscribe((updatedWatchlist) => {
      let updatedWatchlists = this.watchlistService.watchlists.value;
      const watchlistIndex = updatedWatchlists.findIndex(watchlist => watchlist._id === updatedWatchlist._id);
      updatedWatchlists[watchlistIndex] = updatedWatchlist;
      this.watchlistService.watchlists.next(updatedWatchlists);
